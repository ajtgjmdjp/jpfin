...
v0.1.0: Long-only, monthly rebalance, equal-weight, top N by factor.
...
def load_prices_csv(path: str | Path) -> dict[str, PriceData]:
    ...
    with path.open() as f:
        ...
        for row in reader:
            ...
            if not ticker:
                continue
            price_row: dict[str, Any] = {"date": row.get("date", "").strip()}
...
def run_backtest(
    ...
    *,
    ...
    Strategy: At each month-end, compute the specified price-based factor
    for all tickers, select top N, hold equal-weight for the next month.

    Args:
        price_data: Dict mapping ticker → PriceData.
        factor_fn: Factor to rank by. Must be a price-based factor
            ("mom_3m", "mom_12m", "realized_vol_60d", "max_drawdown_252d").
        top_n: Number of top tickers to hold.
    ...
    if n_months > 0 and portfolio_value > 0:
        cagr = portfolio_value ** (12.0 / n_months) - 1.0 if n_months > 0 else 0.0
        import statistics
